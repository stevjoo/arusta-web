    ...
    <div class="bg-darkgray overflow-hidden shadow-sm sm:rounded-lg">
        <div class="sm:p-3 md:p-6 bg-darkgray border-b border-gray-200">
            <div class="my-5 me-3 flex justify-end">
                <a href="{{ route('admin-photography.create') }}" class="px-3 py-2 sm:px-4 sm:py-2 bg-blue-600 hover:bg-blue-800 text-white font-bold rounded-lg shadow transition duration-200">
                    Add New Photo
                </a>
            </div>
                        ...
                        <tr>
                            ...
                            <td class="px-2 sm:px-5 py-2 sm:py-5 border-b border-gray-200 bg-white/10 text-gray-300 text-sm">
                                <div class="flex flex-wrap sm:gap-2">
                                    <a href="{{ route('admin-photography.show', $photo->id) }}" class="px-3 py-2 rounded-lg bg-green-600 hover:bg-green-800 text-white transition duration-200 text-xs sm:text-sm">
                                        View
                                    </a>
                                    <a href="{{ route('admin-photography.edit', $photo->id) }}" class="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-800 text-white transition duration-200 text-xs sm:text-sm">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin-photography.destroy', $photo->id) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-2 bg-red-600 hover:bg-red-800 rounded-lg text-white transition duration-200 text-xs sm:text-sm" onclick="return confirm('Are you sure you want to delete this photo?')">
                                            Delete
                                        </button>
                                    </form>
                                </div>
