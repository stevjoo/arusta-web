...
@section('title', 'Add Graphic Design')
...
<div class="max-w-4xl mx-auto mt-10 border-white border-2 bg-darkgray p-6 rounded-lg shadow-lg text-white">
    ...
    <form method="POST" action="{{ route('admin-graphic-design.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <input 
                type="file" 
                class="w-full px-4 py-2 rounded-lg bg-white border border-gray-400 focus:ring-2 focus:ring-blue-500 focus:outline-none text-black" 
                id="image" 
                name="image" 
                required>
        </div>
        <div class="text-center">
            ...
            <a href="{{ route('admin-graphic-design.index') }}" class="bg-gray-500 hover:bg-gray-700 px-4 py-2 rounded-lg font-medium text-white transition duration-200 ml-4">
                Back to List
            </a>
        </div>
    </form>
